...
public class NFLDScraper {
	//int used to decide between offense (1) and defense anything else

	public static NFLDStats[] Scrape()
	{
		String url = "https://www.espn.com/nfl/stats/team/_/view/defense";

		NFLDStats[] rosterstats = new NFLDStats[32];
		...
		try 
		{
			...
			for (Element row : doc.select("tbody.Table__TBODY tr")) {
				...
				if (row.select("td:nth-of-type(3)").text().equals("")) {
					...
					rosterstats[k] = new NFLDStats(name[k], stats);
					k++;
				}
